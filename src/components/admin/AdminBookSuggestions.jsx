...
export function AdminBookSuggestions() {
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      await updateBookSuggestionStatus(
        statusModal.item._id,
        statusModal.newStatus,
      );
      setSuggestions((prev) =>
        prev.map((item) =>
          item._id === statusModal.item._id
            ? { ...item, status: statusModal.newStatus }
            : item,
        ),
      );
      setStatusModal({
          ...
          <div className="w-full max-w-xl bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-stone-100 flex flex-col max-h-[88vh] overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-4 sm:p-6 pb-4 border-b border-stone-100 shrink-0 bg-white space-y-3">
              ...
              <div>
                ...
                <p className="text-xs font-semibold text-amber-800 pt-0.5">
                  by {viewModal.item.author}
                </p>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] sm:text-[11px] font-bold text-stone-400 uppercase tracking-wider flex items-center gap-1.5">
                  <FileText className="w-3.5 h-3.5 text-stone-400" /> Reader
                  Note
                </label>
                <div className="p-4 sm:p-5 rounded-2xl bg-stone-50/80 border border-stone-200/80 text-xs sm:text-sm text-stone-800 font-serif italic whitespace-pre-wrap leading-relaxed shadow-inner">
                  {viewModal.item.note
                    ? `"${viewModal.item.note}"`
                    : "No note provided."}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                <div className="p-3 sm:p-3.5 rounded-2xl border border-stone-100 bg-stone-50/50 space-y-0.5">
                  <span className="text-[9px] sm:text-[10px] uppercase font-bold text-stone-400 flex items-center gap-1">
                    <User className="w-3 h-3 text-stone-400" /> Suggested By
                  </span>
                  <p className="text-xs font-semibold text-stone-800 truncate">
                    {viewModal.item.suggestedBy || "Anonymous Reader"}
                  </p>
                </div>
                ...
                <div className="p-3 sm:p-3.5 rounded-2xl border border-stone-100 bg-stone-50/50 space-y-0.5">
                  ...
                  <p className="text-xs font-semibold text-stone-800 truncate">
                    {viewModal.item.email ? (
                      <a
                        href={`mailto:${viewModal.item.email}`}
          ...
          {currentItems.map((item) => (
            ...
            >
              <div className="space-y-3">
                ...
                {item.note && (
                  <p className="text-xs text-stone-600 bg-stone-50 p-3 rounded-xl italic border border-stone-100 leading-relaxed line-clamp-3">
                    "{item.note}"
                  </p>
                )}
              </div>
